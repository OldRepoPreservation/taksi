...
// Non-shared codec configuration
COMPVARS g_compVars = {
	0, 0, NULL, 0, 0, NULL, NULL, NULL, NULL,
	0, 0, 0, 0, 0, NULL, 0
};
...
// flag to initialize AVIFile library
BOOL g_aviFileInitDone = FALSE;
...
/* Prints out COMPVARS */
void PrintCompressorSettings()
{
	TRACE2("PrintCompressorSettings: cbSize = %d", (DWORD)g_compVars.cbSize);
	TRACE2("PrintCompressorSettings: dwFlags = %08x", (DWORD)g_compVars.dwFlags);
	TRACE2("PrintCompressorSettings: hic = %08x", (DWORD)g_compVars.hic);
	TRACE2("PrintCompressorSettings: fccType = %08x", (DWORD)g_compVars.fccType);
	TRACE2("PrintCompressorSettings: fccHandler = %08x", (DWORD)g_compVars.fccHandler);
	TRACE2("PrintCompressorSettings: lpbiOut = %08x", (DWORD)g_compVars.lpbiOut);
	TRACE2("PrintCompressorSettings: lKey = %d", g_compVars.lKey);
	TRACE2("PrintCompressorSettings: lDataRate = %d", g_compVars.lDataRate);
	TRACE2("PrintCompressorSettings: lQ = %d", g_compVars.lQ);
}
...
HANDLE OpenAVIFile()
{
	...
	*sizePtr = junkChunkSize - 8;
	...
	// prepare compressor
	memcpy(&g_compVars, &sg_compVars, sizeof(COMPVARS));

	// open compressor
	HIC hic = ICOpen(g_compVars.fccType, g_compVars.fccHandler, ICMODE_FASTCOMPRESS);
	if (hic != NULL)
	{
		TRACE2("OpenAVIFile: ICOpen returned hic = %08x", (DWORD)hic);
		g_compVars.hic = hic;
	}
	...
	g_bmi.bmiHeader.biWidth = g_bbWidth/divider;
	g_bmi.bmiHeader.biHeight = g_bbHeight/divider;
	...
	{
		TRACE2("OpenAVIFile: fccType: {%08x}", icInfo.fccType);
		TRACE2("OpenAVIFile: fccHandler: {%08x}", icInfo.fccHandler);
		WTRACE4("OpenAVIFile: Name: {%S}", icInfo.szName);
		WTRACE4("OpenAVIFile: Desc: {%S}", icInfo.szDescription);
		WTRACE4("OpenAVIFile: Driver: {%S}", icInfo.szDriver);
	}
	else TRACE("OpenAVIFile: ICGetInfo FAILED.");

	ZeroMemory(&g_biOut, sizeof(BITMAPINFOHEADER));
	g_compVars.lpbiOut = &g_biOut;

	// Query compressor to determine its output format
	DWORD formatSize = ICCompressGetFormatSize(g_compVars.hic, &g_bmi);
	TRACE2("formatSize = %d", formatSize);
	DWORD res = ICCompressGetFormat(g_compVars.hic, &g_bmi, g_compVars.lpbiOut);
	if (res == ICERR_OK)
	{
		TRACE("OpenAVIFile: ICCompressGetFormat success.");
		TRACE2("OpenAVIFile: lpbiOut.bmiHeader.biWidth = %d", 
				g_compVars.lpbiOut->bmiHeader.biWidth);
		TRACE2("OpenAVIFile: lpbiOut.bmiHeader.biHeight = %d", 
				g_compVars.lpbiOut->bmiHeader.biHeight);
		TRACE2("OpenAVIFile: lpbiOut.bmiHeader.biCompression = %08x", 
				g_compVars.lpbiOut->bmiHeader.biCompression);
	}
	else TRACE("OpenAVIFile: ICCompressGetFormat FAILED.");

	// print compressor settings
	TRACECALL(PrintCompressorSettings());

	// initialize compressor
	if (ICSeqCompressFrameStart(&g_compVars, &g_bmi))
	{
		TRACE("OpenAVIFile: ICSeqCompressFrameStart success.");
	}
	else TRACE("OpenAVIFile: ICSeqCompressFrameStart FAILED.");
...
void CloseAVIFile(HANDLE hFile)
{
	...
	ICSeqCompressFrameEnd(&g_compVars);
	...
	int padding = (MOVILIST_OFFSET + moviChunkSize + 8) % 2;
	afh.dwRiffChunkSize = MOVILIST_OFFSET + moviChunkSize + 8 + padding + aviIndexChunkSize;
	...
	//afh.streamHeader.fccType = g_compVars.fccType;
	afh.streamHeader.fccHandler = g_compVars.fccHandler;

	afh.streamHeader.dwScale = 1;
	afh.streamHeader.dwRate = frameRate;
	afh.streamHeader.dwLength = totalFrames;
	afh.streamHeader.dwQuality = g_compVars.lQ;
	...
	//memcpy(&afh.biHeader, &g_compVars.lpbiOut->bmiHeader, sizeof(BITMAPINFOHEADER));
	...
	afh.biHeader.biCompression = g_compVars.lpbiOut->bmiHeader.biCompression;
	...
	if (padding > 0) { BYTE zero = 0; WriteFile(hFile, &zero, 1, (LPDWORD)&wbytes, NULL); }
	...
	ICCompressorFree(&g_compVars);
	ICClose(g_compVars.hic);
...
DWORD WriteAVIFrame(HANDLE hFile, BYTE *rgbBuf, SIZE_T size, DWORD times)
{
	...
	int divider = (g_config.fullSizeVideo) ? 1 : 2;
	LONG csize = g_bbWidth/divider * g_bbHeight/divider * 3;
	BOOL isKey = FALSE;
	
	// compress the frame, using chosen compressor.
	BYTE* compressedFrame = (BYTE*)ICSeqCompressFrame(&g_compVars, 0, rgbBuf, &isKey, &csize);
	if (compressedFrame == NULL)
	{
		TRACE("WriteAVIFrame: ICSeqCompressFrame FAILED.");
	}
	else 
	{
		rgbBuf = compressedFrame;
		size = csize;
	}

	TRACE2("WriteAVIFrame: compressed frame size = %d", csize);
	TRACE2("WriteAVIFrame: isKey = %d", isKey);

	for (int i=0; i<times; i++)
	{
		// write frame
		char chunkHeader[] = "00dc\0\0\0\0";
		...
		*(SIZE_T*)(chunkHeader+4) = size;
		...
		currIndex->index[indexedFrames].ckid = 0x63643030; // "00dc"
		currIndex->index[indexedFrames].dwFlags = (isKey) ? AVIIF_KEYFRAME : 0;
...
EXTERN_C _declspec(dllexport) TAXI_CONFIG* GetConfig(void)
{
	return &g_config;
}

EXTERN_C _declspec(dllexport) COMPVARS* GetCompVars(void)
{
	return &sg_compVars;
}
