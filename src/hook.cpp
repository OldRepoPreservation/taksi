...
void ApplySettings(void)
{
	...
	TAXI_CONFIG* cfg = GetConfig();
	cfg->vKeyVideoCapture = config.vKeyVideoCapture;
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	char buf[BUFLEN];
	COMPVARS* pCompVars = NULL;
	TAXI_CONFIG* cfg = NULL;
	...
	switch(uMsg)
	{
		case WM_DESTROY:
			...
			// cleanup VFW resources
			pCompVars = GetCompVars();
			if (pCompVars)
			{
				LOG(log, (log, "pCompVars.fccType = %08x\n", pCompVars->fccType));
				LOG(log, (log, "pCompVars.fccHandler = %08x\n", pCompVars->fccHandler));

				if (pCompVars->cbSize != 0) 
				{
					ICCompressorFree(pCompVars);
					LOG(log, (log, "Compressor resources freed.\n"));
				}
			}
			...
			if (cfg)
			{
				LOG(log, (log, "cfg->useDirectInput = %d\n", cfg->useDirectInput));
			}
		...
		case WM_COMMAND:
			if (HIWORD(wParam) == BN_CLICKED)
			{
				...
				else if ((HWND)lParam == g_codecButtonControl)
				{
					// Display codec-selector dialog
					COMPVARS* pCompVars = GetCompVars();
					if (pCompVars)
					{
						ZeroMemory(pCompVars, sizeof(COMPVARS));
						pCompVars->cbSize = sizeof(COMPVARS);
						ICCompressorChoose(hTaksiWnd, 
							ICMF_CHOOSE_ALLCOMPRESSORS,
							NULL, NULL,
							pCompVars, "Choose compressor");
					}
				}
