...
HWND g_codecButtonControl;          // button to choose codec
...
bool BuildControls(HWND parent)
{
	...
	x = borW/2 + spacer*3/2;
	butW = borW/2 - spacer*3/2;

	g_codecButtonControl = CreateWindowEx(
			xstyle, "Button", "Codec...", style,
			x, y, butW, butH,
			parent, NULL, NULL, NULL);
	...
	if (captureDirectoryLabel == NULL || g_captureDirectoryControl == NULL ||
		...
		frameRateLabel2 == NULL || g_codecButtonControl == NULL ||
	...
	SendMessage(g_codecButtonControl, WM_SETFONT, (WPARAM)hObj, true);
